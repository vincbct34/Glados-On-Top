...
logicalTests = describe "Logical" $ do
  ...
  it "compiles logical or" $ do
    ...
    compileExpr expr
      `shouldBe` [ LOAD_LOCAL (pack "cond1"),
...
stringTests = describe "Strings" $ do
  it "compiles string concatenation" $ do
    ...
    compileExpr expr
      `shouldBe` [ PUSH_STRING (pack "hello"),
...
actorModelTests = describe "Actor Model" $ do
  ...
  it "compiles send with tuple message" $ do
    ...
    compileExpr expr
      `shouldBe` [ LOAD_LOCAL (pack "counter"),
                   PUSH_ATOM (pack "get"),
  ...
  it "compiles Either bind with complex monad" $ do
    let expr =
          EEitherBind
            ...
            (EVar (pack "handler"))
    compileExpr expr
      `shouldBe` [ PUSH_STRING (pack "error"),
                   PUSH_LEFT,
...
processAdvancedTests = describe "Advanced Process Features" $ do
  ...
  it "compiles receive block structure" $ do
    let cases =
          [ Case (PAtom (pack "hello")) (ELiteral (LString (pack "world"))),
            Case (PVar (pack "msg")) (EVar (pack "msg"))
          ]
...
patternMatchingAdvancedTests = describe "Advanced Pattern Matching" $ do
  ...
  it "compiles atom pattern with jump" $ do
    let pat = compilePattern (PAtom (pack "test"))
    pat `shouldSatisfy` (\code -> any (\i -> case i of MATCH_ATOM n _ -> n == pack "test"; _ -> False) code)

  it "compiles integer literal pattern" $ do
    let pat = compilePattern (PLiteral (LInt 42))
    pat `shouldSatisfy` (\code -> any (\i -> case i of MATCH_INT 42 _ -> True; _ -> False) code)

  it "compiles string literal pattern" $ do
    let pat = compilePattern (PLiteral (LString (pack "hello")))
    let helloStr = pack "hello"
    pat `shouldSatisfy` (\code -> any (\i -> case i of MATCH_STRING s _ | s == helloStr -> True; _ -> False) code)

  it "compiles tuple pattern" $ do
    let pat = PTuple [PAtom (pack "get"), PVar (pack "sender")]
