...
-- Glados-On-Top
-- File import Control.Applicative (Alternative (..), empty, many, some, (<|>))
import Data.Bifunctor (first)
import Data.Char (isAlpha, isAlphaNum, isSpace, toUpper)
import Types (LispValue (..))cription:
-- Parser
-}
{-# LANGUAGE LambdaCase #-}
...
instance Functor Parser where
  fmap f p = Parser $ \input ->
    fmap (Bifunctor.first f) (runParser p input)
...
parseWhitespace = void (parseMany parseWhitespaceOrComment)
  where
    parseWhitespaceOrComment =
      void (parseSatisfy isSpace)
        <|> void (parseChar ';' *> parseMany (parseSatisfy (/= '\n')) *> (void (parseChar '\n') <|> parseEOF))

-- | Parse a single comment line
parseComment :: Parser ()
parseComment = void (parseChar ';' *> parseMany (parseSatisfy (/= '\n')))
...
parseBoolean =
  (parseChar '#' *> parseChar 't') $> Boolean True
    <|> (parseChar '#' *> parseChar 'f') $> Boolean False
