...
getBytecode = do
  ...
  when (magic /= BL.pack [0x52, 0x54, 0x42, 0x43]) $
    fail "Invalid magic number - not a Ratatouille bytecode file"

  -- Check version
  verMajor <- getWord8
  verMinor <- getWord8
  when (verMajor /= 1 || verMinor /= 0) $
    fail $
      "Unsupported version: "
        ++ show verMajor
        ++ "."
  ...
  -- Decode instructions
  replicateM (fromIntegral instrCount) decodeInstruction
...
decodeInstruction = do
  ...
  case opcode of
    ...
    0x60 -> do
      ...
      params <- replicateM (fromIntegral paramCount) decodeText
      bodyCount <- getWord32le
      body <- replicateM (fromIntegral bodyCount) decodeInstruction
      return $ DEFINE_PROCESS name params body
    0x61 -> do
      name <- decodeText
      CREATE_INSTANCE name . fromIntegral <$> getWord32le
    0x62 -> return SEND
...
decodeValue = do
  ...
  case tag of
    ...
    0x04 -> do
      count <- getWord32le
      VTuple <$> replicateM (fromIntegral count) decodeValue
    0x05 -> do
      count <- getWord32le
      VArray <$> replicateM (fromIntegral count) decodeValue
...
decodeInteger = do
  ...
  case sizeAndSign of
    ...
    _ -> do
      ...
      bytes <- replicateM (fromIntegral size) getWord8
...
showHex w =
  let (q, r) = w `divMod` 16
      toHex n =
        if n < 10
          then toEnum (fromEnum '0' + fromIntegral n)
