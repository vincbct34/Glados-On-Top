...
compileExpr expr = case expr of
  ...
  EAssign varName assignExpr
    | varName == pack "state" ->
        compileExpr assignExpr ++ [SET_STATE, GET_STATE]
    | otherwise ->
        compileExpr assignExpr ++ [STORE_LOCAL varName, LOAD_LOCAL varName]
  -- Field access
  EFieldAccess targetExpr fieldName ->
    compileExpr targetExpr ++ [GET_FIELD fieldName]
...
compileStmt stmt = case stmt of
  ...
  SLetPattern pat expr -> compileExpr expr ++ compileDestructure pat
...
compileParamBindings params =
  concatMap (\param -> [STORE_LOCAL param]) (reverse params)
...
compileReceiveBlock cases =
  ...
  let dupIfMultiple = [DUP | length cases > 1]
...
compileMatchCasesWithTmp tmpVar [MatchCase pat action] =
  -- Last case: load tmp, pattern + action, no jump needed
  let loadCode = [LOAD_LOCAL tmpVar]
      actionCode = compileExpr action
      patternCode = compilePatternForReceive pat (length actionCode)
   in loadCode ++ patternCode ++ actionCode
compileMatchCasesWithTmp tmpVar (MatchCase pat action : rest) =
  let restCode = compileMatchCasesWithTmp tmpVar rest
      ...
      patternCode = compilePatternForReceive pat failOffset
...
compileReceiveCasesSimple [Case pat action] caseNum =
  ...
  let actionCode = compileExpr action
      ...
      dupInstr = [DUP | caseNum > 0]
   in dupInstr ++ patternCode ++ actionCode
compileReceiveCasesSimple (Case pat action : rest) caseNum =
...
compilePatternForReceive pat failOffset = case pat of
...
compileArrayForReceive patterns failOffset =
  let subPatterns = concatMap (`compilePatternForReceive` 0) patterns
...
compileDestructure pat = case pat of
...
compileIf condition thenBranch elseBranch =
  let condCode = compileExpr condition
      ...
      thenJump = [JUMP (length elseCode) | not (null elseCode)]
...
compileExprStmt expr = case expr of
  ...
  EBinOp op (EVar state) rightExpr
    | state == pack "state" ->
        compileStateAssign op rightExpr
  -- General expression
  _ -> compileExpr expr
