...
handleParseError ::
  ...
  IO ()
handleParseError filePath content err =
  let errMsgStr = errorBundlePretty err
      rich = makeRichError filePath errMsgStr content
   in putStrLnColored "\ESC[1m\ESC[31m" "PARSE ERROR DETECTED"
        >> putStrLn ""
...
compileAST filePath ast mode verbose =
...
compileAndOutput filePath ast mode =
  let bytecode = compileProgram ast
   in putStrLnColored "\ESC[1m\ESC[34m" "COMPILATION PHASE"
...
handleOutputMode _ _ InspectBinary =
  putStrLnColored
    "\ESC[1m\ESC[31m"
...
writeBinaryAndExit inputPath outputPath bytecode =
...
parseOutputArgs = findOutput
  where
    ...
    isKnownFlag x =
      x
        `elem` ["--verbose", "-v", "--show-bytecode", "--show-ast", "--inspect"]
